                ...
                while True:
                    ...
                    if move_best in tabu_move:
                        ...
                        if fit_best>max(tabu_fitness):
                            one_move_solution_list.append(solution_best)
                            one_move_cost_list.append(1/fit_best)
                            one_move_model_list.append(model_bset)
                        ...
                        break
                print("完成了一次初始值")
            finall_cost.append(min(one_move_cost_list))
            index_onemove=one_move_cost_list.index(min(one_move_cost_list))
            finall_solution.append(one_move_solution_list[index_onemove])
            print(finall_solution)
            finall_model.append(one_move_model_list[index_onemove])
        index=finall_cost.index(min(finall_cost))
        finall_best_solution=finall_solution[index]
        finall_best_model=finall_model[index]
        print(len(finall_solution))
        print("best model", "\n", finall_best_model)
...
ins=Instance()
ins.loadData(file)
ins.tabu(iter_times=50,move_times=100, tabu_length=10,candidate_length=30)
